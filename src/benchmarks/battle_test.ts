  ...
  const stream = new ReadableStream({
    pull(controller) {
      for (let i = 0; i < 100 && enqueued < rowCount; i++, enqueued++) {
        let line: string;
        if (Math.random() < 0.01) { // 1% chance of corruption
          const types = ['invalid_json', 'binary_noise', 'broken_literal', 'unterminated'];
          const type = types[Math.floor(Math.random() * types.length)];
          switch (type) {
            case 'invalid_json': line = '{"missing_colon" "value"}\n'; break;
            case 'binary_noise': line = Buffer.from([0x00, 0x01, 0x5b, 0x7b, 0xff]).toString() + '\n'; break;
            case 'broken_literal': line = '{"val": truX}\n'; break;
            case 'unterminated': line = '{"name": "partial...\n'; break;
            default: line = '!!!GARBAGE!!!\n';
          }
        } else {
